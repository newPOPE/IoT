...
static const char *const TAG = "pytes";
static const int MAX_DATA_LENGTH_BYTES = 256;
static const uint8_t ASCII_LF = 0x0A;

PytesComponent::PytesComponent() {}

void PytesComponent::dump_config() {
  this->check_uart_settings(115200, 1, esphome::uart::UART_CONFIG_PARITY_NONE, 8);
  ESP_LOGCONFIG(TAG, "pytes:");
  if (this->is_failed()) {
    ESP_LOGE(TAG, "Connection with pytes failed!");
  }

  for (PytesListener *listener : this->listeners_) {
...
void PytesComponent::setup() {
  ESP_LOGCONFIG(TAG, "Setting up pytes...");
  while (this->available() != 0) {
    this->read();
  }
}

void PytesComponent::update() { this->write_str("pwr\n"); }

void PytesComponent::loop() {
  uint8_t data;

  // pytes sends a lot of data very suddenly
...
void PytesComponent::process_line_(std::string &buffer) {
  ESP_LOGD(TAG, "Read from serial: %s", buffer.substr(0, buffer.size() - 2).c_str());
  // clang-format off
  ...
  PytesListener::LineContents l{};
  const int parsed = sscanf(                                                                                  // NOLINT
      buffer.c_str(), "%d %d %d %d %d %d %d %d %7s %7s %7s %7s %d%% %*d-%*d-%*d %*d:%*d:%*d %*s %*s",  // NOLINT
      &l.bat_num, &l.volt, &l.curr, &l.tempr, &l.tlow, &l.thigh, &l.vlow, &l.vhigh, l.base_st, l.volt_st,     // NOLINT
      l.curr_st, l.temp_st, &l.coulomb);                                                         // NOLINT
...
float PytesComponent::get_setup_priority() const { return setup_priority::DATA; }
