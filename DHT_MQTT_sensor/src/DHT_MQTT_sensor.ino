...
char identifier[24];
#define FIRMWARE_PREFIX "esp8266-dht-temperature-sensor"
...
char MQTT_TOPIC_AUTOCONF_HUMI_SENSOR[128];
char MQTT_TOPIC_AUTOCONF_TIDX_SENSOR[128];
...
void setup() {
  ...
  Serial.println(FIRMWARE_PREFIX);
  ...
  snprintf(identifier, sizeof(identifier), "DHT-%X", ESP.getChipId());
  ...
  snprintf(MQTT_TOPIC_AUTOCONF_HUMI_SENSOR, 127, "homeassistant/sensor/%s/%s_humi/config", FIRMWARE_PREFIX, identifier);
  snprintf(MQTT_TOPIC_AUTOCONF_TIDX_SENSOR, 127, "homeassistant/sensor/%s/%s_tidx/config", FIRMWARE_PREFIX, identifier);
...
void publishAutoConfig() {
  ...
  device["model"] = "ESP8266 - DHT sensor";
  ...
  autoconfPayload["value_template"] = "{{value_json.temperature | round(2, 'ceil') }}";
  ...
  autoconfPayload["value_template"] = "{{value_json.humidity | round(2, 'ceil') }}";
  autoconfPayload["unique_id"] = identifier + String("_humidity");
  autoconfPayload["icon"] = "mdi:water-percent";

  serializeJson(autoconfPayload, mqttPayload);
  mqttClient.publish(&MQTT_TOPIC_AUTOCONF_HUMI_SENSOR[0], &mqttPayload[0], true);
  ...
  autoconfPayload["unit_of_measurement"] = "°C";
  autoconfPayload["value_template"] = "{{value_json.hic | round(2, 'ceil') }}";
  autoconfPayload["unique_id"] = identifier + String("_hic");
  autoconfPayload["icon"] = "mdi:thermometer-auto";

  serializeJson(autoconfPayload, mqttPayload);
  mqttClient.publish(&MQTT_TOPIC_AUTOCONF_TIDX_SENSOR[0], &mqttPayload[0], true);
