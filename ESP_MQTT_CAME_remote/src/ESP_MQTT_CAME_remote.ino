...
char identifier[24];
#define FIRMWARE_PREFIX "esp8266-came"
...
void setup() {
  ...
  snprintf(identifier, sizeof(identifier), "CAME-%X", ESP.getChipId());
  ...
  pinMode(CAME_PIN, OUTPUT);
  digitalWrite(CAME_PIN, HIGH);
}
...
void mqttCallback(char* topic, uint8_t* payload, unsigned int length) {
  ...
  Serial.print("Command triggered");
  //Serial.println(payload);
  Serial.println("");

  digitalWrite(CAME_PIN, LOW);
  delay(250);
  digitalWrite(CAME_PIN, HIGH);
}
...
void publishAutoConfig() {
  ...
  device["model"] = "ESP8266 - CAME";
  device["name"] = identifier;
  device["sw_version"] = "2023.01.0";
